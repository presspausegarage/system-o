...
4. **D4 — Core + Obsidian-augmented.** Spec core = fully operable with any editor; Obsidian layer (Dataview HOME, Canvas, Templater, Kanban render) optional. Requires a static-HOME generator as the Dataview fallback.
   - **Postscript, 2026-07-04**: a full day building a Hyper-V VM specifically to run Obsidian's Electron GUI (kickstart provisioning, DDA GPU passthrough, VT-d, VBS/Memory Integrity — three reboots deep) never got past a display-init hang, likely a Secured-core PC firmware-level VBS enforcement no registry edit could fully undo. That concrete cost is what motivated designating **VS Code + Remote-SSH as the recommended default** (mirrors Andy's day job — VS Code is "the standard" there, a designated default, not a mandate): it needs no GUI app on the container/VM side at all, sidestepping this entire class of problem structurally rather than fighting it. D4's core-works-without-any-editor guarantee doesn't change; this only demotes Obsidian from "the augmented layer's implicit suggestion" to "a fully supported preference, no longer the default lean." See README.md and `reference/docker/README.md` for the updated positioning; `install-obsidian.ps1` still ships, reframed accordingly.
