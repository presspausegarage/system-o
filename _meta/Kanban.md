...
- [x] **First real Docker build + run — the outstanding "never actually built" gap is CLOSED** (2026-07-03, Rocky Linux 9 Hyper-V VM). Found + fixed a real bug the moment the first `docker build` was attempted: `COPY` instructions assumed two incompatible build contexts (`scripts/`/`extensions/`/`templates/` assumed context=`reference/`; `spec/wrap-tail-repair.example.yaml` assumed context=`apps/system-o/` — Docker can't satisfy both, can't reach outside context via `../`). Fixed: context is now consistently `apps/system-o/` (Dockerfile paths prefixed `reference/...`; compose file's `context: ../..` + explicit `dockerfile:`; README corrected). **Full build + run then succeeded clean**: image built, container ran, bootstrap scaffolded the bind-mounted vault correctly (verified on the VM host filesystem, not just container-internal), crontab installed with `$VAULT_ROOT` substituted correctly (the one step Windows couldn't prove), and manually firing the actual cron command (`run-extensions.ps1`) inside the running container reproduced the identical clean result seen in the earlier Windows-side simulation. This is the real v1.0 conformance-test venue (D6) now standing up.

(End v1.0 / pre-public release blockers — return to general Backlog below)
- [ ] Add `tree` command to landing terminal — §File & folder taxonomy is now locked in SPEC.md (confirmed 2026-07-03 during the landing-page reconciliation below), unblocking this
...
- [x] **Dockerfile + first-start bootstrap shipped** (D1/D8) — `reference/docker/{Dockerfile,entrypoint.ps1,bootstrap.ps1,crontab.example,docker-compose.example.yml,README.md}`. pwsh-native (no bash entrypoint); pwsh 7 + cron on Debian, framework baked into `/opt/system-o/`, vault stays a bind mount. Bootstrap scaffolds the full locked taxonomy (§File & folder taxonomy, added same pass — a real gap: the bootstrap needed an answer for "what folders exist" that nothing had written down yet) + starter GLOSSARY/HOME/session-log/orientation file; idempotent (checks for `session-log.md` before scaffolding). Loop layer ships INERT (`.yaml.example`, not `.yaml` — cron-ing placeholder endpoints that fail-closed every night would be noise, not a feature); extensions cron unconditionally (safe, no LLM dependency). **Verified**: both scripts parse-clean and lint-clean; `bootstrap.ps1` rehearsed end-to-end by faking `/opt/system-o` on this Windows box (PowerShell resolves leading `/` as drive-root-relative even on Windows) — full scaffold succeeded, failed only at the genuinely-Linux-only `crontab` binary as expected; idempotency confirmed on re-run; the bootstrapped output was then fed through `build-static-home.ps1` and `run-extensions.ps1` as an integration check, which caught two more real gaps (extension `frontmatter-type` self-flagging its own README.md and the loop prompt template — fixed by adding `extensions`/`templates` to its default exclude pattern). **NOT verified**: no real Docker daemon in this environment — the image has never actually been built or run. Treat the first real `docker compose up` as the true acceptance test (flagged prominently in the docker README).
- [x] **Landing page spec-TOC reconciled with SPEC.md** — `dist/index.html`'s `spec()` command rewritten from the stale 10-section TOC (Operating principles / Folder semantics / Frontmatter rules / Lifecycle state machines / Inbox routing rules / Registry schema / Risk+priority schemas / Agent-instructions cascade / Canonical vocabulary), which predated the loop-cell architecture review (2026-07-01), to the actual 6 committed sections (System architecture, File & folder taxonomy, Agent orientation files, Transform manifest, Loop manifest, Extension surface). `about()`'s "Spec" layer bullet and `help`'s section count updated to match; `V1.0 definition:` line updated to the Docker-primary (Windows+Linux host) + Windows-native-reference conformance matrix decided in D6 (2026-07-02, stub endpoint driver). §ISO alignment left untouched (separate, deliberately partial section). Verified in-browser via preview server — all three commands render correctly. Confirms §File & folder taxonomy is locked in SPEC.md, unblocking the `tree`-command backlog item above — 2026-07-03
